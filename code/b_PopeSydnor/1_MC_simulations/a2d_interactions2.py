...
beta_4_SUP_array = np.array([0, 0.5, 1])#, 5])



## amount of SAP overlap displacement
#displacement_array = np.array([0,0.5,1,1.5,2])#,3,4])
displacement_array = np.array([0.25,0.75,1.25,1.75])#,3,4])


## SUP via Eqn 20.
delta_0_array = np.array([0])
...
for m in binned:
    if m == 'unbinned_SAP':
        writer = pd.ExcelWriter('./output/a2d_interactions_unbinnedSAP_revised' +str(np.int(np.round(10000*random.random())))+'.xlsx', engine='xlsxwriter')
    for k in range(n_rep):
        ...
        for displacement in displacement_array:
            for delta_CP in delta_CP_array:
                        ...
                        for beta_1_SAP in beta_1_SAP_array:
                            for beta_2_CP in beta_2_CP_array:
                                for beta_3_SUP in beta_3_SUP_array:
                                    for beta_4_SAP_SUP in beta_4_SUP_array:
                                        y = beta_1_SAP * SAP + beta_2_CP * CP + beta_3_SUP * SUP + beta_4_SAP_SUP * (SUP * ((SAP + 2)**2) )+ np.random.normal(0,1, N)
                ...
                #                                                                                                             data = df)
                                            ...
                                            beta_4_SAP_SUP_record.append(beta_4_SAP_SUP)
                                            ...
                                            delta_C0_record.append(delta_0)
        ...
        df_results = pd.DataFrame({'RMSE_Full_OLS_train': rmse_full_train,
                                  ...
                                  'Beta4_SAP_SUP':beta_4_SAP_SUP_record
                                  })
