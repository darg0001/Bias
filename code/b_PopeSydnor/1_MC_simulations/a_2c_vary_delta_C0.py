...
## SUP via Eqn 20.
delta_0_array = np.array([-2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2])
delta_CP_array = np.array([0, 1])#, 0.58, 2.07])
...
for m in binned:
    if m == 'unbinned_SAP':
        writer = pd.ExcelWriter('./output/a2c_vary_deltaC0_unbinnedSAP_revised' +str(np.int(np.round(10000*random.random())))+'.xlsx', engine='xlsxwriter')
